  ...
  parseSearchList() {
    ...
    for (const match of matches) {
      ...
      if (tds.length >= 3) {
        ...
        // 歌名和链接（第2列）
        const songTd = tds[1][1];
        const linkMatch = songTd.match(/<a[^>]+href="([^"]+)"[^>]*>([\s\S]*?)<\/a>/i);
        const songUrl = linkMatch ? linkMatch[1] : '';
        const songName = linkMatch ? linkMatch[2].replace(/<[^>]*>/g, '').trim() : '';
        const songId = songUrl ? songUrl.split('/').pop() : '';
        
        // 歌手（第3列）
        const artist = tds[2][1].replace(/<[^>]*>/g, '').trim();
...
async function searchSongs(keyword, page = 1) {
  const url = page === 1 
    ...
    : `${BASE_URL}/s/${encodeURIComponent(keyword)}?page=${page}`;
  ...
  try {
    ...
    // 提取总数
    const parser = new HTMLParser(html);
    const total = parser.parseTotal();
    
    // 提取tbody内容（搜索结果在tbody里）
    ...
    for (const trMatch of trMatches) {
      ...
      // 匹配所有td
      const tdRegex = /<td[^>]*>([\s\S]*?)<\/td>/gi;
      const tdMatches = [...trContent.matchAll(tdRegex)];
      ...
      if (tdMatches.length >= 3) {
        ...
        if (linkMatch) {
          ...
          // 第3列：歌手
          const artist = tdMatches[2][1].replace(/<[^>]*>/g, '').trim();
          
          if (songName && artist && songId) {
            songs.push({
              rank: parseInt(rankText) || songs.length + 1,
              id: songId,
              name: songName,
              artist: artist,
              cover: '',
              url: `${BASE_URL}${songUrl}`
            });
          }
        }
      }
